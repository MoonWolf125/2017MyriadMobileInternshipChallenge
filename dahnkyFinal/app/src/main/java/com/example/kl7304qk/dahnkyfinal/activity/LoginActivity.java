package com.example.kl7304qk.dahnkyfinal.activity;

import android.content.Context;
import android.content.Intent;
import android.support.v7.app.AppCompatActivity;
import android.os.Bundle;
import android.util.Patterns;
import android.view.View;
import android.widget.EditText;
import android.widget.Toast;

import com.example.kl7304qk.dahnkyfinal.API.ApiService;
import com.example.kl7304qk.dahnkyfinal.R;
import com.example.kl7304qk.dahnkyfinal.model.Subscriber;

import org.json.JSONObject;

import java.io.FileOutputStream;
import java.io.InputStream;
import java.util.regex.Pattern;

import retrofit2.Call;
import retrofit2.Callback;
import retrofit2.Response;

public class LoginActivity extends AppCompatActivity {
    private EditText name;
    private EditText email;

    private ApiService apiService;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_login);

        name = (EditText) findViewById(R.id.name);
        email = (EditText) findViewById(R.id.email);

        apiService = new ApiService();
    }

    public void register(View view) {
        if (Patterns.EMAIL_ADDRESS.matcher(email.getText().toString()).matches()) {
            try {
                FileOutputStream outputStream = this.openFileOutput("config", Context.MODE_PRIVATE);
                JSONObject object = new JSONObject();
                object.put("name", name.getText().toString());
                object.put("email", email.getText().toString());
                outputStream.write(object.toString().getBytes());
                outputStream.close();

            } catch (Exception ex) {
                Toast.makeText(getApplicationContext(), ex.getMessage(), Toast.LENGTH_SHORT).show();
            }

            apiService.subscribe(email.getText().toString(), new Callback<Subscriber>() {
                @Override
                public void onResponse(Call<Subscriber> call, Response<Subscriber> response) {
                    Toast.makeText(LoginActivity.this, "Welcome " + name.getText().toString(), Toast.LENGTH_SHORT).show();
                }

                @Override
                public void onFailure(Call<Subscriber> call, Throwable t) {
                    Toast.makeText(LoginActivity.this, "LOGIN: " + t.getMessage(), Toast.LENGTH_LONG).show();

                }
            });
            if (isRegistered(this)) {
                Intent main = new Intent(this, MainActivity.class);
                startActivity(main);
                finish();
            }
        }
        else
            Toast.makeText(this, "Invalid E-mail", Toast.LENGTH_SHORT).show();
    }

    static boolean isRegistered(Context context) {
        boolean registered = false;
        try {
            InputStream is = context.openFileInput("config");
            byte[] buffer = new byte[is.available()];
            is.read(buffer);
            is.close();
            String config = new String(buffer, "UTF-8");
            if (config != null) {
                JSONObject object = new JSONObject(config);
                if (!object.getString("name").equals("") && !object.getString("email").equals(""))
                    registered = true;
            }
        }
        catch (Exception e) {
            try {
                FileOutputStream outputStream = context.openFileOutput("config", Context.MODE_PRIVATE);
                JSONObject object = new JSONObject();
                object.put("name", "");
                object.put("email", "");
                outputStream.write(object.toString().getBytes());
                outputStream.close();
                registered = false;
            }
            catch (Exception ex) {
                Toast.makeText(context, e.getMessage() + " " + ex.getMessage(), Toast.LENGTH_SHORT).show();
            }
        }
        return registered;
    }

    static String getEmail(Context context) {
        String email = "";
        if (isRegistered(context))
            try {
                InputStream is = context.openFileInput("config");
                byte[] buffer = new byte[is.available()];
                is.read(buffer);
                is.close();
                String config = new String(buffer, "UTF-8");
                if (config != null) {
                    JSONObject object = new JSONObject(config);
                    email = object.getString("email");
                }
            }
            catch (Exception e) {
                Toast.makeText(context, e.getMessage(), Toast.LENGTH_SHORT).show();
            }
        return email;
    }

    static int logout(Context context) {
        try {
            FileOutputStream outputStream = context.openFileOutput("config", Context.MODE_PRIVATE);
            JSONObject object = new JSONObject();
            object.put("name", "");
            object.put("email", "");
            outputStream.write(object.toString().getBytes());
            outputStream.close();
            return 1;
        } catch (Exception e) {
            Toast.makeText(context, e.getMessage(), Toast.LENGTH_SHORT).show();
            return 0;
        }
    }
}
